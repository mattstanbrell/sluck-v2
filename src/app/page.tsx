...
export default async function Home() {
  ...
  const { data: workspaceMemberships } = await supabase
    ...
    .limit(1) as { data: WorkspaceMembership[] | null }

  // If user is in at least one workspace, redirect to their first workspace
  if (workspaceMemberships && workspaceMemberships.length > 0 && workspaceMemberships[0].workspaces) {
    ...
    <main className="min-h-screen flex flex-col items-center justify-center">
      <div className="max-w-md w-full px-4">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold mb-2 text-custom-text">Welcome to Sluck</h1>
          <p className="text-custom-text-secondary">Get started by creating a workspace or joining an existing one.</p>
        </div>

        <div className="space-y-8">
          {/* Create Workspace Section */}
          <div className="bg-custom-background-secondary p-6 rounded-lg border border-custom-ui-medium">
            <h2 className="text-xl font-semibold mb-4 text-custom-text">Create a Workspace</h2>
          ...
          <div className="bg-custom-background-secondary p-6 rounded-lg border border-custom-ui-medium">
            <h2 className="text-xl font-semibold mb-4 text-custom-text">Join a Workspace</h2>
            <p className="text-custom-text-secondary mb-4">
